    ...
    with open(filename, "r") as file:
        for line in file:
            ...
            if line_number >= start_line:  # 从数据开始行读取数据
                try:
                    timestamp, thread_name, thread_id, user_usage, kernel_usage = (
                        line.strip().split(",")
                    )
...
def get_summary_table(process_info, thread_info, data):
    ...
    summary_lines.append(
        f"Process ID: {process_info['id']} | Prio: {process_info['priority']}"
    )

    for thread_name in data["thread_name"].unique():
        subset = data[data["thread_name"] == thread_name]

        # 分别检查 user_usage 和 kernel_usage 是否存在
        ...
        if user_exists and subset["user_usage"].sum() == 0 and kernel_exists and subset["kernel_usage"].sum() == 0:
            continue
        # 计算统计数据
        stats = calculate_statistics(subset)

        # 使用get方法来确保即使键不存在也能继续执行
        ...
        if kernel_exists:
            kernel_stats = f"Max/Avg Kernel={stats['max_kernel']}%/{stats['mean_kernel']:.2f}%"

        summary_lines.append(
            f"{thread_name}: Prio={thread_priority} | {user_stats} | {kernel_stats}"
        )
    return "\n".join(summary_lines)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze and plot CPU usage data from a CSV file."
    )
